...
種類:  
- インターフェイスエンドポイント  
    - 他の多くのAWSサービス（EC2、Lambda、OpenSearch等）に使用されます。
    - Elastic Network Interface (ENI) とプライベートIPを使用します。
    - セキュリティグループを関連付けることができます。
    - 使用量に応じて料金がかかります。
    - DNSエントリを作成し、サービスへのトラフィックをプライベートIPにリダイレクトします。
- ゲートウェイエンドポイント  
    - 主にS3とDynamoDBに使用されます。
    - VPCのルートテーブルにエントリを追加することで機能します。
    - インターネットゲートウェイやNATデバイスを必要としません。
    - 料金は無料です。
    - 例：S3へのアクセスに使用されるもの（あなたが使用したタイプ）

OpenSearch用途：  
- インターフェイスエンドポイント  

役割：  
- VPC内からAWSサービスへのプライベートアクセス  
外部からのアクセスには直接関与しない
...
これらのNATをもっと簡略的に説明すると、  
- 主な用途：プライベートサブネット内のリソースからインターネットへのアウトバウンドアクセスを可能にする。
- OpenSearchへのブラウザアクセスには不要
